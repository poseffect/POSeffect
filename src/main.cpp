...
unsigned int nStakeMaxAge = 60 * 60 * 2 * 1;	//2H, stake age of full weight:
...
const string strMessageMagic = "PosEffect Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(pindexBest->nHeight < 6100)
    {
        nSubsidy = 25 * COIN;	
    }
    else if(pindexBest->nHeight < 8100)
    {
        nSubsidy = 12.5 * COIN;
    }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_POSEFFECT_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PosEffect
    bool fStrictPayToScriptHash = true; // Always active in PosEffect
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PosEffect", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "06.03.15 POSEffect Launches";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1425619950;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4241690;
        ...
        assert(block.hashMerkleRoot == uint256("0xdf76e09eb4807cfde347c5fe0911ab4556f8dbd78d284eb76e4e00ddc3e7a604"));
